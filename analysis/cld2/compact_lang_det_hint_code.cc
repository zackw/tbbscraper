...
struct LangTagLookup {
  const char* langtag;    // Lowercased, hyphen only lookup key
  const char* langcode;   // Canonical language codes; two if ambiguous
  OneCLDLangPrior onelangprior1;
  OneCLDLangPrior onelangprior2;
};

struct TLDLookup {
  const char* tld;        // Lowercased, hyphen only lookup key
  OneCLDLangPrior onelangprior1;
  OneCLDLangPrior onelangprior2;
};
...
// In alphabetical order for binary search
static const TLDLookup kCLDTLDHintTable[] = {
  {"ac", JAPANESE + W2, 0},
  {"ad", CATALAN + W4, 0},
  {"ae", ARABIC + W4, 0},
...
static const int kCLDTable3Size =
                   sizeof(kCLDTLDHintTable) / sizeof(kCLDTLDHintTable[0]);
