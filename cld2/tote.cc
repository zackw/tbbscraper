...
void Tote::Add(uint8_t ikey, int idelta) {
  int key_group = ikey >> 2;
  uint64_t groupmask = (1ULL << key_group);
...
void Tote::CurrentTopThreeKeys(int* key3) const {
  ...
  key3[2] = -1;
  int score3[3] = {-1, -1, -1};
  uint64_t tempmask = in_use_mask_;
  int base = 0;
  while (tempmask != 0) {
    if (tempmask & 1) {
...
int DocTote::Find(uint16_t ikey) {
  if (sorted_) {
