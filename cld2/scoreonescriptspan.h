...
struct LangBoosts {
   int32_t n;
   uint32_t langprob[kMaxBoosts];
   int wrap(int32_t n) {return n & (kMaxBoosts - 1);}
};
...
struct LangprobHit {
  uint16_t offset;      // First byte of quad/octa etc. in scriptspan
  uint16_t type;        // LinearHitType
  uint32_t langprob;    // langprob from scoring table
};
...
struct ChunkSummary {
  uint16_t offset;              // Text offset within current scriptspan.text
  uint16_t chunk_start;         // Scoring subscr within hitbuffer->linear[]
  uint16_t lang1;               // Top lang, mapped to full Language
  uint16_t lang2;               // Second lang, mapped to full Language
  uint16_t score1;              // Top lang raw score
  uint16_t score2;              // Second lang raw score
  uint16_t bytes;               // Number of lower letters bytes in chunk
  uint16_t grams;               // Number of scored base quad- uni-grams in chunk
  uint16_t ulscript;            // ULScript of chunk
  uint8_t reliability_delta;    // Reliability 0..100, delta top:second scores
  uint8_t reliability_score;    // Reliability 0..100, top:expected score
};
...
// Score one scriptspan into doc_tote, updating scoringcontext
