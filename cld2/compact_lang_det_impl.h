...
// Internal use flags
static const unsigned int kCLDFlagFinish = 1;
static const unsigned int kCLDFlagSqueeze = 2;
static const unsigned int kCLDFlagRepeats = 4;
static const unsigned int kCLDFlagTop40 = 8;
static const unsigned int kCLDFlagShort = 16;
static const unsigned int kCLDFlagHint = 32;
static const unsigned int kCLDFlagUseWords = 64;
static const unsigned int kCLDFlagUNUSED = 128;
...
***/
  ...
  typedef struct {
    ...
    int kQuadHashB4bShift;
    int kQuadHashB5Shift;
    int kQuadHashB5bShift;
    // Constants for hashing 32 bits to kQuadKeyTable subscript/key
    int kHashvalToSubShift;
    uint32_t kHashvalToSubMask;
    int kHashvalToKeyShift;
    uint32_t kHashvalToKeyMask;
    int kHashvalAssociativity;
    // Pointers to the actual tables
    const PerScriptPair* kPerScriptPair;
    const uint16_t* kQuadKeyTable;
    const uint32_t* kQuadValueTable;
  } LangDetObj;
