...
typedef struct {
  uint32_t keyvalue[4];   // Upper part of word is hash, lower is indirect prob
} IndirectProbBucket4;
...
typedef struct {
  ...
  const uint32_t* kCLDTableInd;       // Each entry is three packed lang/prob
  uint32_t kCLDTableSizeOne;          // Indirect subscripts >= this: 2 entries
  uint32_t kCLDTableSize;             // Bucket count
  uint32_t kCLDTableKeyMask;          // Mask hash key
  uint32_t kCLDTableBuildDate;        // yyyymmdd
