...
CREATE TABLE language_codes (
    ...
    lc3       TEXT NOT NULL UNIQUE CHECK (lc3  <> ''),
    -- Not all languages have ISO 639 alpha-2 codes.
    -- UNIQUE permits more than one row to be NULL.
    lc2       TEXT          UNIQUE CHECK (lc2  <> '')
);
...
CREATE TABLE urls (
    ...
    src    INTEGER NOT NULL REFERENCES url_sources(id),
    meta   JSONB
);
...
CREATE VIEW capture_result (id, result, detail) AS
  SELECT f.id, c.result, f.detail
    FROM capture_fine_result f, capture_coarse_result c
   WHERE f.result = c.id;
