...
def process_one_import(cur, lang, rd, src, datestamp):
    values = []
    for page, url in rd:
        if page == 'page' and url == 'link': continue
        try:
            (uid, _) = add_url_string(cur, url)
            meta = json.dumps({"lang": lang, "page": page})
...
def main():
    ...
    datestamp = time.strftime("%Y-%m-%d",
                              time.gmtime(os.stat(files[0]).st_mtime))


    db = psycopg2.connect(dbname=dbname)
    cur = db.cursor()
    with db:
        srcname = "Wikipedia controversies "+datestamp[:4]
        cur.execute("SET search_path TO collection, public;")
        cur.execute("SELECT id FROM url_sources WHERE name = %s",
                    (srcname,))
        rv = cur.fetchall()
        if rv:
            assert len(rv) == 1
            ...
            cur.execute("INSERT INTO url_sources (name, last_updated, meta)"
                        ...
                        "RETURNING id;",
                        (srcname, datestamp,
                         json.dumps({"url":"http://arxiv.org/abs/1305.5566"})))
            srcid = cur.fetchone()[0]

    xlang = re.compile("^.*_([a-z]{2,3})\.csv$")
    for f in files:
        m = xlang.match(f)
        lang = m.group(1)
        with open(f) as fp:
            ...
            rd = csv.reader(fp)
            with db:
                process_one_import(cur, lang, rd, srcid, datestamp)
