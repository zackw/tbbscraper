...
def process_import(db, listf, datestamp):
    with db, db.cursor() as cur:
        cur.execute("SET search_path TO collection, public;")
        cur.execute("INSERT INTO url_sources (name, last_updated, meta)"
                    "VALUES (%s, %s, %s)"
                    "RETURNING id;",
                    ("Syria "+datestamp[:4]+" (Bluesmote/Telecomix)",
                     datestamp,
                     json.dumps({"url":"http://bluesmote.com/"})))
        src = cur.fetchone()[0]
        values = []
        meta = json.dumps({"country":"sy"})
        for url in listf:
            try: (uid, _ = add_url_string(cur, url.strip())
